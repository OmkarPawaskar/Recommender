...
def generate_rating_dataset(app_label="movies", model="movie"):
    ...
    qs = qs.annotate(userID=F('user_id'), movieId=F("object_id"), rating=F("value")) #https://simpleisbetterthancomplex.com/tips/2016/08/23/django-tip-13-f-expressions.html
...
def export_dataset(app_label='movies', model='movie'):
    ...
    with tempfile.NamedTemporaryFile(model='r+') as temp_f:
        ...
        temp_f.seek(0) #go to top of file
        #write Export model
